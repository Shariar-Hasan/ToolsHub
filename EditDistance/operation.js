...
function editDistance(word1, word2) {
  ...
  while (i < m && j < n) {
    if (word1[i] === word2[j]) {
      ...
      if (dp[i + 1][j + 1] === 1 + dp[i][j]) {
        operations.push(`Exchange '${word1[i]}' with '${word2[j]}'`);
        i++;
        j++;
      } else if (dp[i + 1][j] === 1 + dp[i][j]) {
        operations.push(`Insert '${word2[j]}'`);
        j++;
      } else if (dp[i][j + 1] === 1 + dp[i][j]) {
        operations.push(`Delete '${word1[i]}'`);
        i++;
      }
    }
  }
  ...
  return {
    ...
    similarity: (((Math.max(m, n) - dp[m][n]) / Math.max(m, n)) * 100).toFixed(2),
  };
}
